...
const socials = [
  {
    ...
    metric: 'Followers',
    metricIcon: Users,
    description: 'Behind-the-scenes training footage, fight day prep & lifestyle content.',
  ...
  },
  {
    ...
    metric: 'Subscribers',
    metricIcon: Play,
    description: 'Full training vlogs, sparring sessions, fight breakdowns & journey highlights.',
  ...
  },
  {
    ...
    metric: 'Likes',
    metricIcon: Heart,
    description: 'Short-form training clips, motivation & glimpses of the fighter\'s daily grind.',
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: `radial-gradient(circle at center, ${social.glow} 0%, transparent 70%)`, filter: 'blur(20px)' }}
      />
      ...
      >
        {/* Subtle gradient top */}
        <div className={`absolute top-0 left-0 right-0 h-px bg-gradient-to-r ${social.color} opacity-60`} />

        {/* Header */}
        <div className="flex items-start justify-between mb-5">
          <div className={`p-3 rounded-2xl bg-gradient-to-br ${social.color} text-white shadow-lg`}>
            {social.icon}
          </div>
        ...
        {/* Platform & handle */}
        <div className="mb-4">
          <h3 className="text-lg font-black text-white tracking-tight">{social.platform}</h3>
          <p className={`text-sm font-semibold bg-gradient-to-r ${social.color} bg-clip-text text-transparent`}>
            {social.handle}
          </p>
        </div>

        {/* Description */}
        <p className="text-white/45 text-sm leading-relaxed flex-1 mb-6">{social.description}</p>

        {/* Metric */}
        <div className="flex items-center gap-3">
          <div className={`flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r ${social.color} bg-opacity-10`}
            style={{ background: `linear-gradient(135deg, ${social.glow}, transparent)`, border: `1px solid ${social.glow}` }}>
            <MetricIcon size={14} className="text-white/70" />
            <span className="text-white font-bold text-sm">{social.followers}</span>
            <span className="text-white/50 text-xs">{social.metric}</span>
          </div>
        </div>
      </a>
    </motion.div>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          ...
          <p className="text-white/40 max-w-md mx-auto text-base">
            Join the community. Watch the grind in real-time.
          </p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {socials.map((social, i) => (
            <SocialCard key={social.platform} social={social} index={i} />
          ))}
        </div>
      </div>
    </section>
